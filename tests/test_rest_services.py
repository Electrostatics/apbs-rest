...
def get_job_id():
    response = get('%s/api/uid' % ID_HOST)
...
def post_to_storage(job_id, file_name):
    with open(file_name, 'r') as fin:
        ...
        files = {'file_data': fin }
        response = post('%s/api/storage/%s/%s' % (STORAGE_HOST, job_id, name), files=files)
...
def submit_task(job_id, task_name, task_params=None, task_files=None, use_infile=None):
    if use_infile is True:
        response = post('%s/api/task/%s/%s?infile=%s' % (TASK_HOST, job_id, task_name, str(use_infile)), json=task_params)
    else:
        response = post('%s/api/task/%s/%s' % (TASK_HOST, job_id, task_name), json=task_params)
...
def get_task_status(job_id, task_name, wait=False):
    ...
    response = get('%s/api/task/%s/%s?wait=%s' % (TASK_HOST, job_id, task_name, wait) )
    return response
...
def submit_workflow(job_id, workflow_str, workflow_params=None, task_files=None, use_infile=None):
    if use_infile is True:
        response = post('%s/api/workflow/%s?infile=%s' % (WORKFLOW_HOST, job_id, str(use_infile)), json=workflow_params)
    else:
        request_json = {
            'workflow': workflow_str,
            'form': workflow_params
        }
        response = post('%s/api/workflow/%s' % (WORKFLOW_HOST, job_id), json=request_json)
    return response

def get_workflow_task_status(job_id, task_name, wait=False):
    response = get('%s/api/workflow/%s/%s?wait=%s' % (WORKFLOW_HOST, job_id, task_name, wait) )
    return response
