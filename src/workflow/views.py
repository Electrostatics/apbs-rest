...
import jobutils
import pdb2pqr_runner

# Uncomment storage_service import below if running storage through from same host/port
# from storage import storage_service

from legacy.src.aconf import *
from legacy.src import inputgen
from legacy.src import psize
from legacy import main_cgi # main driver for PDB2PQR
# import querystatus
# import pdb2pqr.main_cgi
from legacy import apbs_cgi # main driver for APBS
...
@workflow_app.route('/home', methods=["GET"])
@workflow_app.route('/pdb2pqr', methods=["GET"])
@workflow_app.route('/apbs', methods=["GET"])
def home():
...
@workflow_app.route('/submit/pdb2pqr', methods=['POST'])
def submit_pdb2pqr():
    """
...
@workflow_app.route('/submit/pdb2pqr/json', methods=['POST'])
...
@workflow_app.route('/submit/apbs', methods=['POST'])
def submit_apbs():
    """
    Handles APBS job submissions.
    Runs the APBS main function originally from 'apbs_cgi.py'.
...
@workflow_app.route('/jobstatus', methods=["GET", "POST"])
def jobstatus():
    """
...
@workflow_app.route('/api/autofill/jobs/<job_type>/<job_id>', methods=['GET'])
...
# @workflow_app.route('/upload', methods=['GET', 'POST'])
@workflow_app.route('/api/upload/autofill/pqr', methods=['POST', 'OPTIONS'])
def upload_autofill():
...
@workflow_app.route('/download/<job_id>/<file_name>', methods=['GET'])
...
# @workflow_app.route('/cli/apbs', methods=['POST'])
@workflow_app.route('/cli/apbs', methods=['POST', 'GET'])
def run_apbs_cli():
...
''' Function used to test piping from a subprocess;
        BE SURE TO REMOVE
'''
@workflow_app.route('/stdout', methods=['GET'])
def test_pipe():
