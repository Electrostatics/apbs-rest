...
def apbsExec(logTime, form, apbsOptions, storage_host):
    ...
    try:
        if form["removewater"] == "on":
            os.chdir('%stmp/%s' % (INSTALLDIR, logTime))
            # os.chdir('./tmp/%s' % logTime)
            inpath = pqrFileName 
            print(os.getcwd())
            infile = open(inpath, "r")
...
def mainInput(form, storage_host) :
    ...
    if form.has_key("jobid"): # means it's not the first run
        ...
        if HAVE_PDB2PQR_OPAL: 
            pdb2pqrOpalJobIDFile = open('%s%s%s/pdb2pqr_opal_job_id' % (INSTALLDIR, TMPDIR, form["jobid"]))
            pdb2pqrOpalJobID = pdb2pqrOpalJobIDFile.read()
            pdb2pqrOpalJobIDFile.close()
            pdb2pqrID = form["jobid"]
            logTime = form["jobid"]
            convertOpalToLocal(form["jobid"], pdb2pqrOpalJobID)
    ...
    if form.has_key("hiddencheck"): # means this time apbs must be run
        firstRun = False
        if form["hiddencheck"] == "local":
            typeOfRun="local"
        elif form["hiddencheck"] == "opal":
            typeOfRun="opal"
